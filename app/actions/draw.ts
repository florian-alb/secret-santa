...
export async function launchDraw() {
  try {
    ...
    if (participants.length < 3) {
      return { success: false, message: 'Il faut au moins 3 participants pour faire un tirage.' }
    }
    ...
    const emailPromises = shuffled.map((giver, index) => {
      ...
      return resend.emails.send({
        from: 'Secret Santa <onboarding@resend.dev>', // À configurer avec un domaine vérifié
        to: giver.email,
    ...
    return { success: true, message: 'Tirage effectué et emails envoyés avec succès !' }

  } catch (error) {
    console.error('Erreur tirage:', error)
    return { success: false, message: "Une erreur s'est produite lors du tirage." }
  }
}
